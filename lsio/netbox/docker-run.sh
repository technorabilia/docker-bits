...
docker run -d \
  ...
  -e BASE_PATH= `# The path you will use to access the app (i.e., /netbox, default: none) [OPTIONAL]` \
  -e CSRF_TRUSTED_ORIGINS= `# List of comma-separated, single quoted, trusted origins. Must include protocol, and port if applicable (default: []) [OPTIONAL]` \
  -e REMOTE_AUTH_ENABLED= `# Enable remote authentication (default: False) [OPTIONAL]` \
  -e REMOTE_AUTH_BACKEND= `# Python path to the custom Django authentication backend to use for external user authentication (default: netbox.authentication.RemoteUserBackend) [OPTIONAL]` \
  -e REMOTE_AUTH_HEADER= `# Name of the HTTP header which informs NetBox of the currently authenticated user. (default: HTTP_REMOTE_USER) [OPTIONAL]` \
  -e REMOTE_AUTH_AUTO_CREATE_USER= `# If true, NetBox will automatically create local accounts for users authenticated via a remote service (default: False) [OPTIONAL]` \
  -e REMOTE_AUTH_DEFAULT_GROUPS= `# The list of groups to assign a new user account when created using remote authentication (default: []) [OPTIONAL]` \
  -e REMOTE_AUTH_DEFAULT_PERMISSIONS= `# A mapping of permissions to assign a new user account when created using remote authentication (default: {}) [OPTIONAL]` \
