...
docker run -d \
  ...
  -e KOMETA_TIMES=03:00 `# Comma-separated list of times to update each day. Format: `HH:MM`. [OPTIONAL]` \
  -e KOMETA_RUN=False `# Set to `True` to run without the scheduler. [OPTIONAL]` \
  -e KOMETA_TESTS=False `# Set to `True` to run in debug mode with only collections that have `test: true`. [OPTIONAL]` \
