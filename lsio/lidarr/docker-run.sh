...
docker run -d \
  ...
  -v ${BASEDIR:-/srv/lsio}/lidarr/config:/config `# Configuration files for Lidarr.` \
  -v ${BASEDIR:-/srv/lsio}/lidarr/music:/music `# Music files (See note in Application setup). [OPTIONAL]` \
  -v ${BASEDIR:-/srv/lsio}/lidarr/downloads:/downloads `# Path to your download folder for music (See note in Application setup). [OPTIONAL]` \
