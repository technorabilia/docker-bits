# [WireGuard®]({{ project_url }}) is an extremely simple yet fast and modern VPN
...
# intends to be considerably more performant than OpenVPN. WireGuard is designed
# as a general purpose VPN for running on embedded interfaces and super
# computers alike, fit for many different circumstances. Initially released for
# the Linux kernel, it is now cross-platform (Windows, macOS, BSD, iOS, Android)
