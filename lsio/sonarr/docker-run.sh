...
docker run -d \
  ...
  -v ${BASEDIR:-/srv/lsio}/sonarr/config:/config `# Database and sonarr configs` \
  -v ${BASEDIR:-/srv/lsio}/sonarr/tv:/tv `# Location of TV library on disk (See note in Application setup) [OPTIONAL]` \
  -v ${BASEDIR:-/srv/lsio}/sonarr/downloads:/downloads `# Location of download managers output directory (See note in Application setup) [OPTIONAL]` \
